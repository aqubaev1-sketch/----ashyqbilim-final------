...
import { GraduationCap, LogIn, Mail, Lock, AlertCircle, ArrowRight, ShieldAlert } from 'lucide-react';
...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || 'Кіру кезінде қате орын алды. Тіркелгі деректерін тексеріңіз.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      setError(err.message || 'Google арқылы авторизациялау сәтсіз аяқталды.');
    }
  };

  const fillDemoCreds = (demoType: 'admin' | 'student') => {
    if (demoType === 'admin') {
      setEmail('admin@educational.platform');
        ...
        <div className="bg-white py-8 px-4 border border-slate-100 shadow-xl rounded-3xl sm:px-10">
          ...
          {success ? (
            <div className="text-center py-8 space-y-3">
              ...
              <h3 className="text-lg font-bold text-slate-900">Авторизация сәтті өтті!</h3>
              <p className="text-sm text-slate-500">Басты бетке бағытталуда...</p>
            </div>
          ) : (
            <form className="space-y-6" onSubmit={handleLogin}>
              ...
              <div className="relative my-6">
                ...
                <div className="relative flex justify-center text-xs uppercase">
                  <span className="bg-white px-3 text-slate-400 font-medium">немесе</span>
                </div>
              </div>
              ...
              <button
                ...
                className="w-full flex justify-center items-center space-x-2 py-3 px-4 bg-white hover:bg-slate-50 border border-slate-200 rounded-xl text-slate-700 font-semibold shadow-sm transition-all text-sm"
              >
